...
def read_split_data(root: str, val_rate: float = 0.2):
    ...
    # 排序，保证各平台顺序一致
    flower_class.sort()  #['daisy', 'dandelion', 'roses', 'sunflowers', 'tulips']
    # 生成类别名称以及对应的数字索引
...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    for data in data_loader:
        ...
        optimizer.step()
        optimizer.zero_grad()
    print("第{}个epoch的损失为:{}".format(epoch + 1, mean_loss.item()))
...
def evaluate(model, data_loader, device,epoch):
    ...
    print("第{}个epoch的准确率是：{}".format(epoch+1,sum_num.item()/total_num))
